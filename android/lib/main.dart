      ...
      if (mounted) {
        Navigator.of(context).pushReplacement(MaterialPageRoute(
          builder: (_) => ControllerScreen(
              connection: conn, deviceName: device.name ?? 'BeastCar'),
        ));
      }
    } catch (e) {
...
class _ControllerScreenState extends State<ControllerScreen>
  ...
  double _speed = 180;
  bool _whiteLed = false;
  bool _blueLed = false;
  bool _ledEffect = false;
  ...
  // Radar
  List<MapEntry<int, int>> _radarData = [];
  ...
  void _listenBluetooth() {
    widget.connection.input!.listen((data) {
      ...
      setState(() {
        ...
        // Parse radar data
        if (msg.startsWith('RADAR:') && msg.contains(',')) {
          final parts = msg.substring(6).split(',');
          if (parts.length == 2) {
            final angle = int.tryParse(parts[0]) ?? 0;
            final dist = int.tryParse(parts[1]) ?? 999;
            _radarData.removeWhere((e) => e.key == angle);
            _radarData.add(MapEntry(angle, dist));
          }
        }
        if (msg == 'RADAR:START') {
          _radarData = [];
  ...
  // ---- TOP BAR ----
  Widget _buildTopBar() {
    final modeData = _modes.firstWhere((m) => m['name'] == _currentMode,
        orElse: () => _modes[0]);
    return Container(
        ...
        Row(children: [
          ...
          Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Text(_connected ? widget.deviceName : 'DISCONNECTED',
                style: GoogleFonts.spaceMono(
                    fontSize: 10,
                    color: _connected ? const Color(0xFF00FF88) : Colors.red)),
          ]),
        ]),
        const Spacer(),
        // Active mode badge
    ...
    return Container(
      ...
      child: Column(children: [
        ...
        Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
          Text('SLOW',
              style:
                  GoogleFonts.spaceMono(fontSize: 10, color: Colors.white24)),
          Text('FAST',
              style:
                  GoogleFonts.spaceMono(fontSize: 10, color: Colors.white24)),
        ]),
      ...
      child: Column(children: [
        ...
        SizedBox(
          ...
          child: GestureDetector(
            onPanStart: (d) {
              ...
              final clamped =
                  raw.distance > maxR ? raw * (maxR / raw.distance) : raw;
              _onJoyUpdate(clamped, maxR);
            },
            onPanUpdate: (d) {
              final c = Offset(size / 2, size / 2);
              final raw = d.localPosition - c;
        ...
        Text(
          ...
          style: GoogleFonts.orbitron(
              ...
              fontWeight: FontWeight.w700,
              color:
                  _lastCmd == 's' ? Colors.white38 : const Color(0xFF00E5FF)),
        ),
      ]),
    );
  }
  ...
  Widget _buildQuickActions() {
    return Row(children: [
      ...
      const SizedBox(width: 12),
      _quickBtn(
          'DANCE', 'J', Icons.music_note_rounded, const Color(0xFFFF4081)),
      const SizedBox(width: 12),
  ...
  Widget _buildLedTab() {
    return SingleChildScrollView(
      padding: const EdgeInsets.all(24),
      child: Column(children: [
        _ledToggleCard('WHITE LED', 'Headlights', Icons.light_rounded,
            Colors.white, _whiteLed, () {
        ...
        Row(children: [
          Expanded(
              child: _bigLedBtn('ALL ON', 'A', Icons.lightbulb_rounded,
                  const Color(0xFFFFD700))),
          const SizedBox(width: 16),
          Expanded(
              child: _bigLedBtn('ALL OFF', 'a', Icons.lightbulb_outline_rounded,
                  Colors.white38)),
        ]),
      ]),
    );
...
class _JoystickPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawCircle(center, maxR + 20, outerPaint);
    ...
    final borderPaint = Paint()
      ...
      ..strokeWidth = 1.5;
    canvas.drawCircle(center, maxR + 20, borderPaint);
    ...
    canvas.drawLine(center + Offset(-(maxR + 20), 0),
        center + Offset(maxR + 20, 0), gridPaint);
    canvas.drawLine(center + Offset(0, -(maxR + 20)),
        center + Offset(0, maxR + 20), gridPaint);
    ...
    final arrowPaint = Paint()
      ...
      ..style = PaintingStyle.fill;
    const a = 7.0;
    final n = center + Offset(0, -(maxR + 10));
    final s2 = center + Offset(0, maxR + 10);
    final l = center + Offset(-(maxR + 10), 0);
    final r = center + Offset(maxR + 10, 0);
    canvas.drawPath(
        Path()
          ..moveTo(n.dx, n.dy - a)
          ..lineTo(n.dx - a, n.dy + a)
          ..lineTo(n.dx + a, n.dy + a)
          ..close(),
        arrowPaint);
    canvas.drawPath(
        Path()
          ..moveTo(s2.dx, s2.dy + a)
          ..lineTo(s2.dx - a, s2.dy - a)
          ..lineTo(s2.dx + a, s2.dy - a)
          ..close(),
        arrowPaint);
    canvas.drawPath(
